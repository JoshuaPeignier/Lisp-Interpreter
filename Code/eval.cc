...
Object eval(Object l, Environment env) {
  ...
  if (symbolp(f)) {
	...
	if(size(l) > 2){
		clog << "Error : too many fields in car\n" << endl;
		return eval(nil(),env);
	}
	else if(size(l) < 2){
		clog << "Error : not enough fields in car\n" << endl;
		return eval(nil(),env);
	}
	Object elem = cadr(l);
	return eval(car(elem),env);
    }
    if(Object_to_string(f) == "cdr"){
	if(size(l) > 2){
		clog << "Error : too many fields in cdr\n" << endl;
		return eval(nil(),env);
	}
	else if(size(l) < 2){
		clog << "Error : not enough fields in cdr\n" << endl;
		return eval(nil(),env);
	}
	Object elem = cadr(l);
	return cdr(elem);
	...
	if(size(l) > 3){
		clog << "Error : too many fields in cons\n" << endl;
		return eval(nil(),env);
	}
	else if(size(l) < 3){
		clog << "Error : not enough fields in cons\n" << endl;
		return eval(nil(),env);
	}
	Object elem = cadr(l);
	Object tail = caddr(l);
	assert(!listp(elem));
	assert(listp(tail) || is_empty(l));
	return cons(eval(elem,env),tail);
    }
  }
