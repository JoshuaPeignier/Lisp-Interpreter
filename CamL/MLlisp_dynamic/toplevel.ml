...
let handle_set obj = 
	...
	in
	begin
		print("SET: " ^ defined_name ^ " = ");
		print(defining_value);
		print_newline();
		global_env_ref := update_env_modify defined_obj defining_value !global_env_ref
	end
;;
...
let handle_directive obj toplevel =
  if (listp obj) then (* listp is defined in objects.ml ; returns true if obj is a list, else false*)
