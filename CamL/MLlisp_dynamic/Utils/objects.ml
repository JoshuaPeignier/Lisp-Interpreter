...
let obj_to_string obj =
  ...
  | _ -> error2 obj "Not an symbol nor a string nor a number"
;;
...
let car obj =
  match obj with
  | Lisp_list(x::_) -> x
  | _ -> error2 obj "Cannot take car of such an object"
;;

let cdr obj =
  match obj with
  | Lisp_list(_::m) -> Lisp_list(m)
  | _ -> error2 obj "Cannot take cdr of such an object"
;;

let cons obj lobjs=
  match lobjs with
  | Lisp_list(l) -> Lisp_list(obj::l)
  | _ -> error2 lobjs "Cannot cons to such an object"
;;
...
let subr_to_fun obj =
  match obj with
  | Lisp_subr(f) -> f
  | _ -> error2 obj "Cannot use such an object as a subr"
;;
